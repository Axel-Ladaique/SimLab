# Symlab — Sub-project 1: Core Simulator Design
...
Symlab is a realistic RC airplane simulator. Its primary goal is **training transfer**: the pilot
...
├─ tests/Symlab.Input.Tests/   # xUnit
└─ docs/
```
...
2. `Symlab.Input` converts raw values to normalized channels in [-1, 1].
3. `Symlab.Flight` advances the simulation by N fixed sub-steps of **2 ms (500 Hz)**, using an
