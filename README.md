...
- `src/SimLab.App` — testable game logic: flying fields (map model with the club field), three camera views (ground line-of-sight, FPV,
  chase), procedural aircraft meshes, input routing, flight session, settings, translations.
- `game/` — Godot 4 (.NET) simulator: radio setup and calibration, flying fields (map model with the club field), flying from the ground, FPV or
