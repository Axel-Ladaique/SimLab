...
/// <summary>
/// Layout of the generic club field in world ENU axes (x east, y north, z up). The runway runs east-west, centered on
/// the origin; the pilot box is south of it.
/// </summary>
public static class ClubField
{
    ...
    public static readonly Vec3 PilotPosition = new(0, -25, 0);
    public static readonly Vec3 WindsockPosition = new(20, -28, 0);

    public static bool OnRunway(double x, double y) => Math.Abs(x) <= RunwayLength / 2 && Math.Abs(y) <= RunwayWidth / 2;
    ...
    public static (double X, double Y) TakeoffPoint(double headingDeg) =>
        headingDeg == 90 ? (-RunwayLength / 2 + TakeoffInset, 0) : (RunwayLength / 2 - TakeoffInset, 0);

    /// <summary>A few meters north of (in front of) the pilot.</summary>
    public static (double X, double Y, double HeadingDeg) HandLaunchPoint(double windFromDeg) =>
        (PilotPosition.X, PilotPosition.Y + HandLaunchDistance, TakeoffHeading(windFromDeg));
