...
/// sets over it. Both the height and the ground mix blend from the grid edge's over the first
/// <see cref="EdgeBlend"/> metres, so the ring meets the terrain without a step or a colour seam.
/// </summary>
public sealed class MountainBackdrop(HeightGrid grid, MountainSurface surface)
{
    const double EdgeBlend = 1500;
    ...
    public double Height(double x, double y)
    {
        ...
        return edge + (Far(x, y) - edge) * MountainSurface.SmoothStep(Beyond(x, y) / EdgeBlend);
    }

    /// <summary>
    /// Ground mix at (x, y) outside the grid: rock on steep ground, snow above +900 m, needles below +500 m and grass
    /// between, blended from the map's own mix at the nearest grid edge point.
    /// </summary>
    public SurfaceWeights Surface(double x, double y)
    {
        ...
        var far = SurfaceWeights.Only(SurfaceKind.Grass)
            .Toward(SurfaceKind.Needles, 1 - MountainSurface.SmoothStep((z - 450) / 100))
            .Toward(SurfaceKind.Snow, MountainSurface.SmoothStep((z - 850) / 100))
